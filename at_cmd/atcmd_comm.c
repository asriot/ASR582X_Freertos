...
int lega_at_heap(int argc, char **argv)
{
    at_rspinfor("left free heap %d\r\n",xPortGetFreeHeapSize());
    return CONFIG_OK;
}
...
cmd_entry comm_heapshow = {
    .name = "heapshow",
    .help = "cmd format:   heapshow\r\n\
cmd function: show left free heap\r\n",
    .function = lega_at_heap,
};

void lega_at_comm_cmd_register(void)
{
    lega_at_cmd_register(&comm_help);
    lega_at_cmd_register(&comm_heapshow);
