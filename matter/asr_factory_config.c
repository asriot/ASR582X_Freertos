...
int32_t asr_factory_config_data_search(asr_matter_partition_t matter_partition, uint8_t * buf, uint32_t *pBufLen, uint32_t *puiOffset)
{
  ...
  do
  {
    ...
    ret = duet_flash_read(ASR_CONFIG_BASE, (uint32_t *) &usOffset, (void *)ucParaName, ASR_CONFIG_NAME_SIZE);
    if (ret != 0)
    {
      printf("duet_flash_read ASR_CONFIG_NAME_SIZE failed\r\n");
      return ret;
    }

    ret = duet_flash_read(ASR_CONFIG_BASE, (uint32_t *) &usOffset, (void *) &uiDataLen, ASR_CONFIG_LEN_SIZE);
    if (ret != 0)
    {
      printf("duet_flash_read ASR_CONFIG_LEN_SIZE failed\r\n");
      return ret;
    }
  ...
  *pBufLen = uiDataLen;
  ...
  ret = duet_flash_read(ASR_CONFIG_BASE, (uint32_t *) &usOffset, (void *) buf, uiDataLen);
...
int32_t asr_factory_config_write(uint8_t * configbuffer, uint32_t buf_len)
{
    ...
    return duet_flash_erase_write(ASR_CONFIG_BASE, (uint32_t *) &off_set, (void *) configbuffer, buf_len);
}
                                        ...
                                        uint32_t buf_len)
{
    ...
    ret = duet_flash_read(ASR_CONFIG_BASE, (uint32_t *) &uiOffset, (void *) &uiDataLen, ASR_CONFIG_LEN_SIZE);
    if (ret != 0)
    {
    ...
    return duet_flash_erase_write(ASR_CONFIG_BASE, (uint32_t *) &uiOffset, (void *) buf, buf_len);
}
extern int32_t duet_factory_decrypt_dac_prvkey(uint8_t *pDacCipher, uint8_t ucDacKeyLen, uint8_t *pRdBuf, uint32_t *pOutLen);
int32_t asr_factory_dac_prvkey_get(uint8_t *pRdBuf, uint32_t *pOutLen)
{
  ...
  // get DAC cipher
  ret = duet_flash_read(ASR_CONFIG_BASE, (uint32_t *) &off_set, (void *) ucCipher, 32);
  if (ret != 0)
    return ret;
  ...
  if (ret != 0)
  {
    ret = duet_factory_decrypt_dac_prvkey(ucCipher, 32, pRdBuf, pOutLen);
    if (ret != 0)
    {
...
uint8_t asr_factory_check()
{
  ...
  ret = duet_flash_read(ASR_CONFIG_BASE, (uint32_t*)&uiOffset, ucRdBuf, uiRdLen);
  if(ret != 0)
  {
