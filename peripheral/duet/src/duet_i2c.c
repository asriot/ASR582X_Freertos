...
SEG_I2C int32_t duet_i2c_master_send(duet_i2c_dev_t *i2c, uint16_t dev_addr, const uint8_t *data, uint16_t size, uint32_t timeout)
{
    int32_t ret = 0;
    I2C_TypeDef * I2Cx;
    uint16_t temp;
            ...
            for(temp = 0; temp < size; temp++)
            {
                ...
                else
                {
                    if(temp == size - 1)
                    {
                        ...
                        while( !i2c_get_flag_status(I2Cx, I2C_STATUS_TRANS_DONE) )
                        {
                            if(g_duet_i2c_timeout)
                            {
                                g_duet_i2c_timeout = 0;
...
SEG_I2C int32_t get_fifo_size(void)
{
    int32_t wptr = I2C0->WFIFO_WPTR, rptr = I2C0->WFIFO_RPTR;
    int32_t fifo_size = wptr - rptr;
    if(fifo_size < 0)
        fifo_size += 16;
    return fifo_size;
}
SEG_I2C int32_t duet_i2c_master_recv(duet_i2c_dev_t *i2c, uint16_t dev_addr, uint8_t *data, uint16_t size, uint32_t timeout)
{
    I2C_TypeDef * I2Cx;
    uint16_t i = 0;
    int32_t ret = 0;
...
int32_t duet_i2c_master_repeated_write_read(duet_i2c_dev_t *i2c, uint8_t slave_addr, const uint8_t * pwdata, uint8_t * rdata, uint32_t wlen, uint32_t rlen, uint32_t timeout)
{
    /**** master write ****/
    int32_t ret = 0;
    uint8_t temp;
